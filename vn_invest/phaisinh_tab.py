        ...
        with st.expander("📂 Lịch sử Journal (file CSV)", expanded=False):
            try:
                ...
                for _, r in df_j.iterrows():
                    ...
                    rows_j += (
                        f"<tr>"
                        f"<td style='color:#888;white-space:nowrap'>{r.get('time','')}</td>"
                        f"<td class='{a_cls}'>{act}</td>"
                        f"<td style='color:#fff;font-weight:700'>{r.get('price','')}</td>"
                        f"<td>{sl_str}</td>"
                        f"<td>{tp_str}</td>"
                        f"<td class='{p_cls}'>{pnl_v}</td>"
                        f"<td style='color:#666;font-size:11px'>{str(r.get('reason',''))[:80]}</td>"
                        f"</tr>"
                    )
                st.markdown(
                    "<style>"
                    ".jn-tbl{width:100%;border-collapse:collapse;font-size:13px}"
                    ".jn-tbl th{background:#1a1d24;color:#aaa;padding:8px 12px;text-align:left;border-bottom:2px solid #333;white-space:nowrap}"
                    ".jn-tbl td{padding:8px 12px;border-bottom:1px solid #222;vertical-align:middle}"
                    ".jn-tbl tr:hover td{background:#1e2130}"
                    ".c-long{color:#00E676;font-weight:700} .c-short{color:#FF5252;font-weight:700} .c-wait{color:#888}"
                    ".c-pos{color:#00B0FF;font-weight:700} .c-neg{color:#FFD600;font-weight:700}"
                    "</style>"
